 ...
 * @constructor
 */
function WalletCollection (config) {
    /**
     * @type {Object}
     * @private
     */
    ...
    /** @type {Currency} */
    this.baseCurrency = new Currency({walletCollection: this, currency: this.app.settings.get("base.currency")});

    /** @type {Currency} */
    this.mhcCurrency = new Currency({walletCollection: this, currency: CURRENCY_ID_MHC});
 ...
 * @return {Promise<*>}
 */
WalletCollection.prototype.initNativeWallets = function () {
    return new Promise(function (resolve, reject) {
        bridgeCallHandler("getWallets", {email: this.app.user.email})
            .then(function (data) {
 ...
 */
WalletCollection.prototype.loadRates = function () {
    apiXhr(getJsonRpcXhrParams("currency.stat", {params: [{type: "last"}]}))
 ...
 * @return {number}
 */
WalletCollection.prototype.getRate = function (baseCurrency, currency) {
 ...
 * @todo переписать на metagate.js
 */
WalletCollection.prototype.loadRatesHistory = function () {
    let time = {
        day: "time24",
        week: "week",
        month: "month",
    ...
    apiXhr(getJsonRpcXhrParams("currency.stat", {params: [{type: time}]}))
        .then(function (response) {
            if (response.result === API_OK) {
                this.history = {};
                ...
                for (let currencyId in response.data) {
                    ...
                    let history = {data: [], first: 0, last: 0, diff: 0, diffPercent: 0, low: 0, high: 0};
 ...
 * @todo переписать на metagate.js
 */
WalletCollection.prototype.load = function (forceLoad) {
    ...
    this.initNativeWallets()
        .then(function () {
            ...
            apiXhr(getJsonRpcXhrParams("address.list", {token: this.app.user.token}))
                .then(function (response) {
                    if (response.result === API_OK) {
                        this._time = Date.now();
                        ...
                        sortedData.forEach(function (/** @type {MetaGateWallet} */ metagateWallet) {
                            ...
                            if (!this.currencies[metagateWallet.currency]) {
                                this.currencies[CURRENCY_CODES[metagateWallet.currency]] = new Currency({walletCollection: this, currency: metagateWallet.currency});
                            } else {
                            ...
                            if (!this.wallets[metagateWallet.address]) {
                                ...
                                this.wallets[metagateWallet.address] = this.nativeWallets[metagateWallet.address]
                                    ? Wallet.fromMetaGateAndNativeWallets({walletCollection: this}, metagateWallet, this.nativeWallets[metagateWallet.address])
                                    : Wallet.fromMetaGateWallet({walletCollection: this}, metagateWallet);
                            } else {
                        ...
                        for (let address in this.nativeWallets) {
                            ...
                            if (!this.wallets[address]) {
                                ...
                                this.wallets[address] = Wallet.fromNativeWallet({walletCollection: this}, this.nativeWallets[address]);
                            }
                        }

                        for (let currencyId of CURRENCIES) {
                            if (currencyId !== DEFAULT_BASE_CURRENCY && !this.currencies[CURRENCY_CODES[currencyId]]) {
                                let currency = new Currency({walletCollection: this, currency: currencyId});
 ...
 * @param {Wallet} wallet
 */
WalletCollection.prototype.onWalletLoaded = function (wallet) {
...
WalletCollection.prototype.onTransactionsLoaded = function () {
 ...
 * @return {Array<Wallet>}
 */
WalletCollection.prototype.getWalletsSortedByTime = function (currencyId) {
    /** @type {Array<Wallet>} */
    let wallets = [];
    if (currencyId) {
 ...
 */
WalletCollection.prototype.getTransactionsSortedByTime = function (currencyId) {
