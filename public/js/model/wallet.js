 ...
 */
class Wallet {
    constructor (config, metagateWallet, nativeWallet, blockchainWallet, newWallet) {
        /**
         * @type {Object}
         * @private
         */
        ...
        this.currency = new Currency({
            walletCollection: this.walletCollection,
    ...
    fetchBalance (refresh) {
        ...
        if (this.currencyId === CURRENCY_ID_ETH || this.currencyId === CURRENCY_ID_BTC) {
            apiXhr(getJsonRpcXhrParams("address.balance", {
                ...
                .then(function (response) {
                    if (response.result === API_OK && response.data.length === 1) {
                        ...
                        Events.trigger("Wallet.onBalanceFetched", {
                            wallet: this
                        });
    ...
    fetchTransactions (refresh) {
        ...
        if (this.currencyId === CURRENCY_ID_ETH || this.currencyId === CURRENCY_ID_BTC) {
            apiXhr(getJsonRpcXhrParams("address.transaction", {
                    token: this.app.user.token,
                    params: [{
                        address: this.address,
                        currency: this.currencyId
                ...
                .then(function (response) {
                    if (response.result === API_OK) {
                        ...
                        this._transactionsFetched = true;
                        Events.trigger("Wallet.onMoreTransactionsFetched", {
                            wallet: this,
        ...
        metahashApi.fetchHistory({
                address: this.address,
                beginTx: beginTx,
                countTxs: TRANSACTIONS_PER_FETCH
            })
            .then(function ( /** @type {Array<MetaHashTransaction>} */ result) {
                ...
                if (!refresh || transactions.length) {
                    Events.trigger("Wallet." + eventCode, {
                        wallet: this,
                        transactions: transactions
                    });
                }
    ...
    getDelegated () {
        ...
        let delegated = {
            ...
            localeString: (delegatedAmount / CURRENCY_COEFFICIENT[this.currencyId]).toLocaleString("en", {
    ...
    getRewards () {
        ...
        xhr(getJsonRpcXhrParams("fetch-history-filter", {
                params: {
                    address: this.address,
                    filters: {
                        isForging: true
            ...
            .then(function (response) {
                if (response.result) {
                    response.result.txs.forEach(function ( /** @type {MetaHashTransaction} */ transaction) {
                        this.rewardsTransaction.push(new Transaction({
                            walletCollection: this.walletCollection,
                            currency: this.currency,
                            wallet: this,
                    ...
                    Events.trigger("Wallet.onRewardsTransactionsFetched", {
                        wallet: this
                    });
                }
            }.bind(this))
     ...
     * @return {Array<Transaction>}
     */
    getRewardsTransactionsSortedByBlockNumber () {
                            ...
                            response.result.states.forEach(function ( /** @type {MetaHashDelegation} */ state) {
                                if (state.to !== ADDRESS_FORGING && state.to !== ADDRESS_NODE_REGISTRATOR) {
                                    if (typeof this.delegations[state.to] === "undefined") {
                                        this.delegations[state.to] = new Delegation({
                                            wallet: this,
                                            state: state
                                        });
                                    } else {
                                    ...
                                    this.delegationTransactions.push(new Delegation({
                                        wallet: this,
                                        state: state
                                    }));
                                }
                            }.bind(this));
                            Events.trigger("Wallet.onDelegationsFetched", {
                                wallet: this
                            });
                            resolve();
                        }
    ...
    addToMetaGate () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            apiXhr(getJsonRpcXhrParams("address.create", {
                    token: this.app.user.token,
                    params: [apiParams]
                }))
                .then(function () {
    ...
    rename (newName) {
        return new Promise(function (resolve, reject) {
            ...
            apiXhr(getJsonRpcXhrParams("address.name.set", {
                    token: this.app.user.token,
                    params: [apiParams]
                }))
                .then(function () {
     ...
     * @return {Wallet}
     */
    static fromMetaGateWallet (config, metaGateWallet) {
        /** @type {Wallet} */
        ...
        /** @type {Currency} */
        wallet.currency = new Currency({
            walletCollection: wallet.walletCollection,
     ...
     * @return {Wallet}
     */
    static fromNativeWallet (config, nativeWallet) {
        /** @type {Wallet} */
        let wallet = new Wallet(config);
        /** @type {?NativeWallet} */
     ...
     * @return {Wallet}
     */
    static fromMetaGateAndNativeWallets (config, metaGateWallet, nativeWallet) {
        /** @type {Wallet} */
        let wallet = new Wallet(config);
        /** @type {MetaGateWallet} */
        wallet.metagateWallet = metaGateWallet;
     ...
     * @return {Wallet}
     */
    static newWallet (config, blockchainWallet, newWallet) {
        /** @type {Wallet} */
        let wallet = new Wallet(config);
        /** @type {number} */
     ...
     * @return {Wallet}
     */
    static blankWallet (config, currencyId) {
        /** @type {Wallet} */
        let wallet = new Wallet(config);
        /** @type {number} */
