 ...
 * @constructor
 */
class TransactionSubView extends View {
    constructor (config) {
    ...
    onStarted () {
        ...
        xD(this.element.querySelector(".transactions-log-card")).bind("click", function ( /** @type {MouseEvent|TouchEvent} */ event) {
            this.app.alertView(new TransactionView( /** @type {ViewConfig} */ {
                    app: this.app,
        ...
        this.element.qs("actions.info").bind("click", function () {
            bridgeCallHandler("openUrl", {
                url: EXPLORER_ENDPOINT_TX[this.currencyId] + this.transaction.hash
            });
        }.bind(this), this.app);
    }
    update () {
