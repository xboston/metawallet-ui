...
class CurrenciesView extends View {
    ...
    onShown () {
        ...
        bridgeCallHandler("getView")
            .then(function (data) {
                ...
                let view = new window[data.view]( /** @type {ViewConfig} */ Object.assign({
                    app: this.app
                }, data.params || {}));
                switch (view.type) {
                    case VIEW_PAGE:
    ...
    drawCurrencySubViews () {
        ...
        for (let currency of Object.values(this.app.walletCollection.currencies)) {
            ...
            if (this.__firstDraw) {
                TweenMax.fromTo(currencySubView.element, .1, {
                    opacity: 0
                }, {
                    opacity: 1
                });
     ...
     */
    getCurrencySubView (currency) {
        /** @type {CurrencySubView} */
        let currencySubView = new CurrencySubView({
            app: this.app,
            element: this._currencyTemplate.cloneNode(true),
            currency: currency
        });
    ...
    getWalletSubView (wallet) {
        ...
        let walletSubView = new WalletSubView({
            app: this.app,
            element: this._walletTemplate.cloneNode(true),
            wallet: wallet
        });
        walletSubView.start();
        return walletSubView;
