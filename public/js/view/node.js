...
class NodeView extends View {
    ...
    update () {
        ...
        this.element.qs("actions.delegate").onclick = function () {
            // тут промис
            showDelegate(this.app, {
                node: this.node,
                currencyId: CURRENCY_ID_MHC
            });
        }.bind(this);
        ...
        this.element.qs("actions.info").onclick = function () {
            bridgeCallHandler("openUrl", {
                url: EXPLORER_ENDPOINT_ADDRESS[CURRENCY_ID_MHC] + this.node.address
            });
        }.bind(this);
    }
    ...
    getDelegationSubView (delegation) {
        ...
        let delegationSubView = new DelegationNodeSubView({
            app: this.app,
            element: template,
