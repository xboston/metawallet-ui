 ...
 * @constructor
 */
class QrCodeAlertView extends View {
    constructor (config) {
    ...
    saveAddressQrCode () {
        bridgeCallHandler("shareImage", {
            data: this.addressQrElement.toDataURL()
        });
    }
    showPkQrCode () {
        if (this.loader) {
    ...
    savePkQrCode () {
        bridgeCallHandler("shareImage", {
            data: this.pkQrElement.toDataURL()
        });
    }
}